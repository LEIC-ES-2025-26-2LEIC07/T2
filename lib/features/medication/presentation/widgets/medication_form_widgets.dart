...
class MedTimePickerSheet extends StatelessWidget {
  const MedTimePickerSheet({
    super.key,
    required this.current,
    required this.onSelected,
    required this.onPickCustom,
    this.userSchedules = const [],
  });

  final TimeOfDay current;
  final ValueChanged<TimeOfDay> onSelected;
  final VoidCallback onPickCustom;
  final List<TimeOfDay> userSchedules;

  static const _suggestions = [
    (label: 'Manhã', time: (h: 8, m: 0)),
    (label: 'Meio-dia', time: (h: 12, m: 0)),
    (label: 'Tarde', time: (h: 15, m: 0)),
    (label: 'Noite', time: (h: 20, m: 0)),
    (label: 'Deitar', time: (h: 22, m: 0)),
    (label: 'Cedo', time: (h: 6, m: 0)),
  ];

  Widget _chip(BuildContext context, TimeOfDay t, {String? label}) {
    final isSelected = current.hour == t.hour && current.minute == t.minute;
    return GestureDetector(
      onTap: () => onSelected(t),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
        decoration: BoxDecoration(
          color: isSelected ? medBlue : medCard,
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: isSelected ? medBlue : medInk, width: 2),
          boxShadow: const [medShadowSm],
        ),
        child: Column(
          children: [
            if (label != null)
              Text(
                ...
                style: TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.w700,
                  color: isSelected ? Colors.white : medMuted,
                ),
              ),
            ...
            Text(
              ...
              style: TextStyle(
                fontSize: 15,
                fontWeight: FontWeight.w800,
                color: isSelected ? Colors.white : medInk,
              ),
            ),
          ],
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (userSchedules.isNotEmpty) ...[
            ...
            Text(
              'OS SEUS HORÁRIOS HABITUAIS',
              style: TextStyle(
                fontSize: 11,
                fontWeight: FontWeight.w800,
                letterSpacing: 1.2,
                color: medInk.withValues(alpha: 0.55),
              ),
            ),
            ...
            Wrap(
              spacing: 10,
              runSpacing: 10,
              children: userSchedules.map((t) => _chip(context, t)).toList(),
            ),
            const SizedBox(height: 16),
            Text(
              'SUGESTÕES',
          ...
          Wrap(
            ...
            children: _suggestions.map((s) {
              final t = TimeOfDay(hour: s.time.h, minute: s.time.m);
              return _chip(context, t, label: s.label);
            }).toList(),
          ),
          const SizedBox(height: 20),
          SizedBox(
