...
class AppBackground extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      color: const Color(0xFFF4F4F4), // Slightly off-white background matching the image
      child: CustomPaint(
...
class TopographicPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      path.cubicTo(
        getSizeX(size, 0.3), startY + 150 + (i * 5),
        getSizeX(size, 0.6), startY - 50 + (i * 2),
        size.width, startY + 100,
      );
      ...
      path2.cubicTo(
        getSizeX(size, 0.4), startY2 - 200 - (i * 5),
        getSizeX(size, 0.7), startY2 + 100 - (i * 2),
        size.width, startY2 - 50,
      );
    ...
    for(int i = 0; i < 5; i++) {
        canvas.drawCircle(Offset(size.width * 0.8, size.height * 0.7), 50.0 + (i * 40), paint);
    }
  }
